...
def create_masks(request: CompletedRequest) -> Dict[int, np.ndarray]:
    ...
    if np_outputs is None:
        return res
    _log_detection_csv('segmentation', 1.0)
    mask = np_outputs[0]
...
if __name__ == "__main__":
    # cv2 est importé indirectement (via picamera2) et réécrit
    # QT_QPA_PLATFORM_PLUGIN_PATH vers son propre plugin Qt embarqué,
    # incompatible avec le preview natif Picamera2 lancé plus bas.
    # On l'annule ici, une fois les imports faits, pour laisser
    # Picamera2 utiliser le plugin Qt système.
