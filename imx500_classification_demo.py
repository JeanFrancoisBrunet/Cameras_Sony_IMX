...
_last_log_time = 0.0
_LOG_INTERVAL  = 5.0   # secondes entre deux lignes de log

def _log_detection_csv(label: str, score: float, bbox=None):
    global _last_log_time
    log_path = _os.environ.get("IMX500_LOG_CSV", "")
    if not log_path:
        return
    now = _time.time()
    ...
    try:
        with open(log_path, "a", newline="") as _f:
            csv.writer(_f).writerow(
                [ts, label, f"{score:.3f}", str(bbox) if bbox else ""])
    except Exception:
        pass
...
def draw_classification_results(request: CompletedRequest, results: List[Classification], stream: str = "main"):
    ...
    with MappedArray(request, stream) as m:
        ...
        for index, result in enumerate(results):
            ...
            _log_detection_csv(label, result.score)
...
if __name__ == "__main__":
    # cv2 est importé plus haut (pour le dessin des résultats) et
    # réécrit QT_QPA_PLATFORM_PLUGIN_PATH vers son propre plugin Qt
    # embarqué, incompatible avec le preview natif Picamera2 lancé
    # plus bas. On l'annule ici, une fois les imports faits, pour
    # laisser Picamera2 utiliser le plugin Qt système.
    import os as _os_qtfix
    _os_qtfix.environ.pop("QT_QPA_PLATFORM_PLUGIN_PATH", None)
